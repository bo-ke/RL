# -*- coding: utf-8 -*-
# !/usr/bin/env python3
"""Sync files and commands across multiple nodes.
...
def get_ips():
    """Get all IP addresses from hostfile."""
...
def concat_cmd(cmd):
    """Concatenate command parts with proper quoting."""
    return " ".join([quote(c) for c in cmd])


def sync_file(path):
    """Sync file to all nodes."""
...
def sync_cmd(cmd):
    """Sync and execute command on all nodes."""
    self_ip = get_self_ip()
    ips = get_ips()
