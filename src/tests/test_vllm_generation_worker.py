...
class TestWorkerInitializer:
    ...
    def create_worker(self, num_gpus=1, bundle_indices=None, **options):
        ...
        # 配置选项
        worker_options = {"num_gpus": num_gpus, **options}
        # 创建 worker（异步）
        worker = self.worker_class.options(**worker_options).remote(
            config=self.config, **worker_kwargs
        )
        print("  [Initializer] Worker creation call completed")
        return worker
...
class VllmGenerationWorkerTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        initializer = TestWorkerInitializer.options(name="test_initializer").remote(
        ...
        print(
            f"  ✓ Worker creation call completed in {time.perf_counter() - worker_start:.3f}s"
        )
        print("    (Worker.__init__ is running in background...)")
        ...
        print(
            f"  ✓ Verification completed in {time.perf_counter() - verify_start:.2f}s"
        )
